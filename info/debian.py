...
class Debian(DistroBaseInfo):
    ...
    def kernel_cmdline(self) -> list[string]:
        return [
            "root=/dev/vda1",
            "ro",
        ]  # bad, but Debian does not label either the partition or the filesystem, so we've to hardcode it
        ...
        self.qcow2_hrefs = list(
            set(
                [  # make unique, there's duplicate links
                    ...
                    if href.endswith(".qcow2")
                    and ("-" + self.slug + "-") in href
                    and ("-" + self.distro.variant + "-") in href
                ]
            )
        )
