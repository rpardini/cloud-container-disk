...
class DistroBaseInfo:
    ...
    def __init__(
        self,
        arches: list[DistroBaseArchInfo],
        default_oci_ref_disk: string,
        default_oci_ref_kernel: string,
    ):
        ...
        self.oci_ref_disk = os.environ.get(
            "DISK_OCI_REF",
            os.environ.get("BASE_OCI_REF", "ghcr.io/rpardini/") + default_oci_ref_disk,
        )
        self.oci_ref_kernel = os.environ.get(
            "KERNEL_OCI_REF",
            os.environ.get("BASE_OCI_REF", "ghcr.io/rpardini/") + default_oci_ref_kernel,
        )
    ...
    def get_oci_def_kernel(self) -> MultiArchImage:
        ...
        for arch in self.arches:
            image.create_kernel_image(
                arch.docker_slug,
                arch.vmlinuz_final_filename,
                arch.initramfs_final_filename,
            )
        return image
